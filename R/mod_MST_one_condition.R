...
mod_MST_one_condition_ui <- function(id){
  ...
  tagList(
    sidebarLayout(
      ...
      fileInput(ns("MST_expresion"),
                accept = c('text/csv',
                           'text/comma-separated-values,text/plain',
                           '.csv'),
                label = h5("Gene Expression Data")),
      fileInput(ns("MST_Genes"),
                accept = c('text/csv',
                           'text/comma-separated-values,text/plain',
                           '.csv'),
...
mod_MST_one_condition_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    filedata_MST_expression <- reactive(
      {
        ...
        fileInput <- load_file(input$MST_expresion$name, 
                               input$MST_expresion$datapath)
    ...
    filedata_MST_genes <- reactive({
      req(input$MST_Genes)
      fileInput <- load_file(input$MST_Genes$name, 
                             input$MST_Genes$datapath)
      fileInput <- as.data.frame(fileInput)
      return(list(fileInput = fileInput))
    })
