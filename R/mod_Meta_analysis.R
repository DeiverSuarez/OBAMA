    ...
    sidebarLayout(
      ...
      fluidRow(
        ...
        column(6,
               ...
               conditionalPanel(
                 condition ="input.PMs111==3",
                 ns=ns,
                 column(6,
                        style=list("padding-right: 28px;"),
                        textInput(ns("ValueqPM11"),
                                  label = h5("Quantile value (q)"),value = 75)
                        )
                 )
               ),
        fluidRow(
          column(4,
                 textInput(ns("NFro_11"),
      ...
      conditionalPanel(
        ...
        downloadButton(
          ns("downloadData.2metanalisis"),
          "Save My Result")
        ),
      
      conditionalPanel(
        condition = "input.Cases==3",
        ns=ns,
        fileInput(
          ns("fileBcsv113"),
          accept = c('text/csv',
                     'text/comma-separated-values,text/plain',
                     '.csv'),label = h5("Gene Expression Data 1")
          ),
        ...
        fileInput(
          ...
          accept = c('text/csv',
                     'text/comma-separated-values,text/plain',
                     '.csv'),label = h5("Gene Expression Data 3")
          ),
        ...
        downloadButton(
          ns("downloadData.3metanalisis"), 
          "Save My Result")
        ),
      
      conditionalPanel(
        condition = "input.Cases==4",
        ns=ns,
        fileInput(
          ns("fileBcsv114"),
          accept = c('text/csv',
        ...
        actionButton(
          ...
          border-color: #2e6da4"),
        downloadButton(
          ns("downloadData.4metanalisis"),
          "Save My Result")
        ),
      
      conditionalPanel(
        condition = "input.Cases==5", 
        ns=ns,
        fileInput(
          ns("fileBcsv115"),
          accept = c('text/csv',
        ...
        downloadButton(ns("downloadData.5metanalisis"),
                       "Save My Result")
        ),
      ),
      mainPanel(
        conditionalPanel(
          condition = "input.Cases== 2 || input.Cases== 3",
          ns=ns,
          tabsetPanel(
        ...
        conditionalPanel(
          condition = "input.Cases== 4 || input.Cases== 5",
          ns=ns,
          tabsetPanel(
            tabPanel(
              "SummaryData",
...
mod_Meta_analysis_server <- function(id)
  ...
  moduleServer( 
      ...
      filedata_m1 <- reactive(
        {
          ...
          fileInput <- load_file(input$fileBcsv112$name, 
                                 input$fileBcsv112$datapath)
      ...
      filedata_m11 <- reactive(
        {
          ...
          fileInput <- load_file(input$fileBcsv122$name, 
                                 input$fileBcsv122$datapath)
          fileInput <- as.data.frame(fileInput)
          return(list(fileInput = fileInput))
          }
        )
       ...
       filedata_m3 <- reactive(
         {
           ...
           fileInput <- load_file(input$fileBcsv113$name,
                                  input$fileBcsv113$datapath)
           fileInput <- as.data.frame(fileInput)
           return(list(fileInput = fileInput))
           }
         )
       ...
       filedata_m32 <- reactive(
         {
           ...
           fileInput <- load_file(input$fileBcsv123$name, 
                                  input$fileBcsv123$datapath)
           fileInput <- as.data.frame(fileInput)
           return(list(fileInput = fileInput))
           }
         )
       ...
       filedata_m33 <- reactive(
         {
           ...
           fileInput <- load_file(input$fileBcsv133$name, 
                                  input$fileBcsv133$datapath)
           fileInput <- as.data.frame(fileInput)
           return(list(fileInput = fileInput))
           }
         )
       ...
       filedata_m41 <- reactive(
         {
           ...
           fileInput <- load_file(input$fileBcsv114$name,
                                  input$fileBcsv114$datapath)
           fileInput <- as.data.frame(fileInput)
           return(list(fileInput = fileInput))
           }
         )
    ...
    filedata_m42 <- reactive(
      {
        ...
        fileInput <- load_file(input$fileBcsv124$name, 
                               input$fileBcsv124$datapath)
        fileInput <- as.data.frame(fileInput)
        return(list(fileInput = fileInput))
        }
      )
    ...
    filedata_m43 <- reactive({
      ...
      fileInput <- load_file(input$fileBcsv134$name,
                             input$fileBcsv134$datapath)
      fileInput <- as.data.frame(fileInput)
      return(list(fileInput = fileInput))
    }
    )
    ...
    filedata_m44 <- reactive({
      ...
      fileInput <- load_file(input$fileBcsv144$name,
                             input$fileBcsv144$datapath)
      fileInput <- as.data.frame(fileInput)
      return(list(fileInput = fileInput))
    })
    ...
    filedata_m51 <- reactive({
      ...
      fileInput <- load_file(input$fileBcsv115$name,
                             input$fileBcsv115$datapath)
      fileInput <- as.data.frame(fileInput)
      return(list(fileInput = fileInput))
    })
    ...
    filedata_m52 <- reactive({
      ...
      fileInput <- load_file(input$fileBcsv125$name, 
                             input$fileBcsv125$datapath)
      fileInput <- as.data.frame(fileInput)
      return(list(fileInput = fileInput))
    })
    ...
    filedata_m53 <- reactive({
      ...
      fileInput <- load_file(input$fileBcsv135$name,
                             input$fileBcsv135$datapath)
      fileInput <- as.data.frame(fileInput)
      return(list(fileInput = fileInput))
    })
    ...
    filedata_m54 <- reactive({
      ...
      fileInput <- load_file(input$fileBcsv145$name,
                             input$fileBcsv145$datapath)
      fileInput <- as.data.frame(fileInput)
      return(list(fileInput = fileInput))
    })
    ...
    filedata_m55 <- reactive({
      ...
      fileInput <- load_file(input$fileBcsv155$name,
                             input$fileBcsv155$datapath)
      fileInput <- as.data.frame(fileInput)
      return(list(fileInput = fileInput))
    })
    ...
    Stady2_mdf <- eventReactive(input$button112,{
      if(input$Cases == 2) {
          ...
          q=input$ValueqPM11
          outEval <- mco_two_meta_analysis(data1 = data1, 
                                           data2 = data2, 
                                           NF=NF, 
    ...
    Stady3_mdf <- eventReactive(input$button113,{
      if(input$Cases == 3) {
        ...
        outEval <- mco_three_meta_analysis(data1 = data1,
                                           data2 = data2, 
                                           data3 = data3, 
                                           NF=NF, 
                                           measurePM_m = measurePM_m, 
    ...
    Stady4_mdf <- eventReactive(input$button114,{
      if(input$Cases == 4) {
        ...
        outEval <- mco_four_meta_analysis(data1 = data1,
                                          data2 = data2,
                                          data3 = data3, 
                                          data4 = data4,
                                          NF=NF, 
    ...
    Stady5_mdf <- eventReactive(input$button115,{
      if(input$Cases == 5) {
        ...
        outEval <- mco_five_meta_analysis(data1 = data1, 
                                          data2 = data2, 
                                          data3 = data3,
                                          data4 = data4, 
                                          data5 = data5,
