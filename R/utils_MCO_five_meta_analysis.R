MCO_five_meta_analysis <- function(data1,
                                   data2,
                                   data3, 
                                   data4, 
                                   data5, 
  ...
  MCO <- function(M1,M2,M3,M4,M5,z=NULL){
    ...
    Gamma_m1 <- replace(
      alpha,alpha==0 | alpha==W |  alpha==2*W | alpha==3*W | alpha==4*W, 5/2*W)
  ...
  MCO_partition <- function(M1,M2,M3,M4,M5, h=NULL){
    ...
    M5_zzz=split(ZZZ,as.numeric(gl(length(ZZZ),
                                   trunc(length(ZZZ)/m),
                                   length(ZZZ)))) 
