MCO_four_meta_analysis <- function(data1, 
  ...
  MCO <- function(M1,M2,M3,M4,z=NULL){
    W=1000
  ...
  MCO_partition <- function(M1,M2,M3,M4,h=NULL){
    X=M1;Y=M2;Z=M3;ZZ=M4
    ...
    #print("######position######")
    #print(position)
    f1=MCO(X[position],Y[position],Z[position],ZZ[position])
    #print("######name gen########")
    r=unlist(front)[f1]
    print(r)
  ...
  MCO_general <- function(M1,M2,M3,M4){
    F_acu=list(c())
    fr=list(c())
    f1=MCO_partition(M1,M2,M3,M4)
    F_acu[[1]]=f1
    fr[[1]]=f1
    
    for (i in 2:NF) {
      fr[[i]]=MCO_partition(M1,M2,M3,M4,h=c(F_acu[[i-1]]))
  ...
  N_front=MCO_general(M1,M2,M3,M4)
